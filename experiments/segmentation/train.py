...
class Trainer():
    def __init__(self, args):
        ...
        if hasattr(model, 'jpu') and model.jpu is not None:               #if hasattr(model, 'jpu'):
            params_list.append({'params': model.jpu.parameters(), 'lr': args.lr*10})
        if hasattr(model, 'head') and model.head is not None:             #if hasattr(model, 'head'):
            params_list.append({'params': model.head.parameters(), 'lr': args.lr*10})
        if hasattr(model, 'auxlayer') and model.auxlayer is not None:     #if hasattr(model, 'auxlayer'):
            params_list.append({'params': model.auxlayer.parameters(), 'lr': args.lr*10})
        optimizer = torch.optim.SGD(params_list, lr=args.lr,
            momentum=args.momentum, weight_decay=args.weight_decay)
        # criterions
        self.criterion = SegmentationLosses(se_loss=False, aux=False,
                                            nclass=self.nclass, 
    ...
    def training(self, epoch):
        ...
        for i, (ir_img, vis_img, target) in enumerate(tbar):
            ...
            print("Loss value:", loss)  # 检查损失是否为None
    ...
    def validation(self, epoch):
        ...
        def eval_batch(model, ir_img,vis_img, target):
        ...
        for i, (ir_img, vis_img, target) in enumerate(tbar):
            ...
            if torch_ver == "0.3":
                ir_img = Variable(ir_img, volatile=True)
                vis_img = Variable(vis_img, volatile=True)
                correct, labeled, inter, union = eval_batch(self.model, ir_img,vis_img, target)
            else:
                with torch.no_grad():
                    correct, labeled, inter, union = eval_batch(self.model, ir_img,vis_img, target)
            print(f"correct: {correct}, labeled: {labeled}, inter: {inter}, union: {union}")
            ...
            print(f"pixAcc: {pixAcc}, mIoU: {mIoU}")
